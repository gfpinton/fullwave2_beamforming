...
basedir='/kulm/scratch/fullwave2_beamforming_vishuman_planewave'
%%% Advanced variables %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
tx.dd=focusDelays(tx.focs,xdc.incoords,cfl,0);

for n=1:tx.nTx
  tx.dd(:,n)=tx.dd(:,n)-mean(tx.dd(:,n));
end
tx.dd=tx.dd-min(min(tx.dd));
...
%%% GENERATE IMAGE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 for ii=1:length(lats)
   ...
   pxducer_now=pxducer2(:,:,ii);
   for jj=1:length(deps)
     bm2(ii,jj)=sum(pxducer_now(round(rx.idps_mat_rx(ii,jj,2:rx.idps_mat_rx(ii,jj,1))+rx.idps_mat_ax(ii,jj)+idt0s(ii))));
   end
...
figure(1)
subplot(1,4,3)
imagesc(lats*1e3,deps*1e3,dbah(squeeze(bm2')),[-45 0])
colormap gray, colorbar
xlabel('mm'), ylabel('mm')
axis equal, axis tight
subplot(1,4,4)
...
figure(6)
subplot(1,2,1)
imagesc(lats*1e3,deps*1e3,pwah(interp2easy(bm',interpfac,interpfac),1/3),[-50 0])
colormap gray, colorbar
xlabel('mm'), ylabel('mm')
axis equal, axis tight
subplot(1,2,2)
